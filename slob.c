...
/*
 * Lab 3 arrays to store claimed and free 
 */
long claimed_mem[100];
long free_mem[100];
int counter=0;
...
/*
 * Allocate a slob block within a given slob_page sp and keep track of best block for a best fit implementation.
 */
static void *slob_page_alloc(struct page *sp, size_t size, int align)
{
	...
	//lab 3
	slob_t * best_cur=NULL, *best_aligned=NULL, best_prev=NULL;
	int best_delta=0;
	...
	for (prev = NULL, cur = sp->freelist; ; prev = cur, cur = slob_next(cur)) {
		...
		if (slob_last(cur)) // last block of page
		{
			if (best_cur!=NULL) 
			{
				...
				sp->units -= units; // new available units page
				if (!sp->units)
...
/*
LAB 3 HELPER FUNCTION TO FIND BEST page by returning the free leftover space of the given sp
*/
static int slob_page_fit(struct slob_page *sp, size_t size, int align)
{
	slob_t *prev, *cur, *aligned = NULL;
	int delta = 0, units = SLOB_UNITS(size);
...
/*
 * slob_alloc: entry point into the slob allocator modified to do best fit allocation.
 */
...
// lab 3
	...
	{	 	temp_free+=sp->units;
		...
		if (sp->units < SLOB_UNITS(size))
			continue;
		current_fit=slob_page_fit(sp,size,align);
		if(current_fit==0)
		{
			best_page=sp;
	...
	if(best_fit>=0) //if found smallest possible page
	{
		/* Attempt to alloc */
	...
	if (!b) {
		...
		//lab 3
		claimed_mem[counter]=size;
		free_mem[counter]=(temp_free * SLOB_UNIT) - SLOB_UNIT + 1;
